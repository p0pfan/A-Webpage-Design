...
CKEDITOR.plugins.add('scayt', {
	...
	init: function(editor) {
		...
		var config = editor.config,
			...
			env = CKEDITOR.env;
		plugin.state[editor.name] = true;
		
		
		editor.addCommand('enablescayt', {
			exec : function(editor) {
				...
				if(plugin.state[editor.name] === true) {
					...
					editor.getCommand( 'enablescayt' ).setState( CKEDITOR.TRISTATE_ON );
				} else {
					if(scaytInstance) {
						plugin.destroy(editor);
					}
					editor.getCommand( 'enablescayt' ).setState( CKEDITOR.TRISTATE_OFF );
				}
			}
		});
